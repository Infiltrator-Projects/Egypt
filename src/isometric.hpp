...
struct IsoCamera{
    ...
    // Pharaoh-style screen handedness: increasing map X runs down-left,
    // increasing map Y runs down-right. The previous projection was the
    // horizontal mirror of this, which made the Nile/map read backwards.
    [[nodiscard]] IsoPoint project(int tx,int ty,int elevation=0) const {
        const int hw=tile_w()/2, hh=tile_h()/2;
        return {origin_x+pan_x+(ty-tx)*hw,
                origin_y+pan_y+(tx+ty)*hh-elevation};
    }
    ...
    [[nodiscard]] bool pick(int sx,int sy,int& tx,int& ty) const {
        ...
        const double a=(sx-(origin_x+pan_x))/hw; // ty - tx
        const double b=(sy-(origin_y+pan_y))/hh; // tx + ty
        const double fx=(b-a)*0.5;
        const double fy=(a+b)*0.5;
